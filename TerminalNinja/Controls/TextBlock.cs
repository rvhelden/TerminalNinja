...
public sealed class TextBlock : FrameworkElement
{
    ...
    private void RenderText(CellBuffer buffer, int x, int y, int width, int height)
    {
        ...
        for (var i = 0; i < processedLines.Count && startY + i < y + height; i++)
        {
            ...
            // Render characters. Step 7 dispatched shaped runs from here; Step 8 moved that
            // to Renderer.Present so it works for every control that writes cells, not just
            // TextBlock. We just write per-cell cells and the renderer reconstructs runs
            // from contiguous same-style cells on the shaped path.
    ...
    private void RenderInlineLine(CellBuffer buffer, List<InlineRun> runs, int x, int y, int width, int height, int totalLength)
    {
        ...
        //
        // Shaped rendering (HarfBuzz ligatures, complex scripts) is driven by Renderer.Present
        // reconstructing runs from contiguous same-style cells, not by this control. That keeps
        // the dispatch logic in one place and benefits every control that writes cells.
        var charIndex = 0;
