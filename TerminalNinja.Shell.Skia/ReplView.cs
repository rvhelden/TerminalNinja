...
public sealed class ReplView : Control
{
    ...
    public override void OnMouseEvent(MouseEvent e)
    {
        ...
        //
        // When the mouse hover is open with overflow content, it claims the
        // wheel first so the user can browse a long obj.dump payload without
        // the output history scrolling behind the popup. Single-row hovers
        // (ForwardScroll returns false) fall through to output scrolling.
        if (e.Action == MouseAction.ScrollUp)
        {
            if (_mouseHover.ForwardScroll(-3)) { Invalidate(); return; }
