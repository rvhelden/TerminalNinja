...
internal sealed class HoverBox : FrameworkElement
{
    ...
    /// <summary>
    /// Mouse-wheel scroll. Positive <paramref name="delta"/> moves toward later
    /// content (wheel-down), negative toward earlier (wheel-up). Returns true
    /// when the hover claimed the event — true whenever the content overflows,
    /// even at a scroll boundary, so the wheel keeps acting on the hover instead
    /// of leaking through to the output history behind it. Single-row hovers
    /// return false so the wheel falls through naturally.
    /// </summary>
