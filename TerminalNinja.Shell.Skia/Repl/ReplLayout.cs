    ...
    int PromptWidth)
{
    ...
    public static ReplLayout Compute(
        ...
        int promptWidth)
    {
        // The input region grows downward from a baseline near the bottom: it
        // always occupies inputLines rows (>= 1), and the optional diagnostic
        // row sits directly above it. Clamp inputLines to half the panel height
        // so a runaway multi-line buffer can't swallow the entire output area.
        ...
        var topReserved = inputLines + (diagY > -1 ? 1 : 0);
        var outputHeight = Math.Max(0, bounds.Height - topReserved);

        return new ReplLayout(bounds, outputHeight, inputTopY, inputLines, diagY, promptWidth);
    }
}
